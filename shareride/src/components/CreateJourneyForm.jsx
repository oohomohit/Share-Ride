...
function CreateJourneyForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      [journeyStartLocation, journeyEndLocation, journeyDescription, journeyDate, journeyTime].some(field => field === '') ||
      passengerRequired === 0 ||
      fare === 0
    ) {
      ...
      <div className="bg-white shadow-xl rounded-lg p-10 max-w-3xl w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              className={`bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={loading}
            >
